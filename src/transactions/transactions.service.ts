...
import { FilterValidTransaction, FilterInvalidTransaction, FilterResult } from './interfaces/filter.interface';
...
export class TransactionsService {
    ...
    filter(dto: FilterRequestDto): FilterResult {
        ...
        for (let i = 0; i < n; i++) {
            ...
            if (t.amount < 0) {
                invalid.push({ date: t.date, amount: t.amount, message: APP_MESSAGES.TRANSACTIONS.FILTER.NEGATIVE_AMOUNT });
                continue;
            }
            const key = `${t.date}__${t.amount}`;
            if (seen.has(key)) {
                invalid.push({ date: t.date, amount: t.amount, message: APP_MESSAGES.TRANSACTIONS.FILTER.DUPLICATE });
            } else {
                seen.set(key, true);
                candidates.push({ date: t.date, amount: t.amount, ceiling: t.ceiling, remanent: t.remanent, inKPeriod: false });
            }
        }
        ...
        if (dto.q.length > 0) {
            const sortedQ = dto.q
                ...
                .sort((a, b) => a.startMs !== b.startMs ? b.startMs - a.startMs : a.idx - b.idx);
        ...
        if (dto.p.length > 0) {
            ...
            interface PEvent { ts: number; delta: number; }
            const events: PEvent[] = [];
            ...
            const order = Array.from({ length: m }, (_, i) => i)
                .sort((a, b) => candidateMs[a] - candidateMs[b]);
